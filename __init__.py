...
#记录赠送女友数据，同一群内不同交易可并发
class SendJudger:
    def __init__(self):
        self.send_dict = {}
        self.on = {}
    def turn_on_off(self, gid, uid1, uid2, on_off=False):
        self.on[str(gid)+str(uid1)+str(uid2)] = on_off
    def get_status(self, gid, uid1, uid2):
        return self.on[str(gid)+str(uid1)+str(uid2)] if self.on.get(str(gid)+str(uid1)+str(uid2))!=None else False
    def accept_on_off(self, gid, uid1, uid2, on_off=False):
        self.send_dict[str(gid)+str(uid1)+str(uid2)] = on_off
    def get_accept_status(self, gid, uid1, uid2):
        flag = self.send_dict.get(str(gid)+str(uid1)+str(uid2))
        return self.send_dict[str(gid)+str(uid1)+str(uid2)] if flag!=None else None

send_judger = SendJudger()
...
#随机生成一个pcr角色id
def get_pcr_id(gid):
    chara_id_list = [k for k in _pcr_data.CHARA_NAME.keys() if (re.match(f"\d+{gid}",str(k)) or k<10000)] ##pcr角色或者本群群友
    while True:
...
@sv.on_fullmatch('叮咚一下')
async def noblelogin(bot, ev: CQEvent):
    gid = ev.group_id
    uid = ev.user_id
    if not lmt.check(uid):
        await bot.send(ev, '今天已经叮咚过了哦，明天再来吧。', at_sender=True)
        return
    duel = DuelCounter()
    ...
    lmt.increase(uid)
    score_counter._add_score(gid, uid ,500)
...
async def add_noble(bot, ev: CQEvent):
    try:
        ...
        if duel._get_level(gid, uid)!= 0:
            ...
            score_counter = ScoreCounter2()
            lmt.increase(uid)
            score_counter._add_score(gid, uid ,300)
...
@sv.on_fullmatch(['查询贵族','我的贵族','贵族查询'])   
...
async def duelrefuse(bot, ev: CQEvent):
    ...
    uid = ev.user_id
    if duel_judger.get_on_off_accept_status(gid):
        if uid == duel_judger.get_duelid(gid)[1]:
            score_counter = ScoreCounter2()
            score_counter._reduce_score(gid, uid ,10) 
            msg = '您已拒绝贵族决斗，因为有失贵族尊严扣除您10金币'
            await bot.send(ev, msg, at_sender=True)
            duel_judger.turn_off_accept(gid)
...
async def leave_girl(bot, ev: CQEvent):
    ...
    if owner == 0 or int(owner)!=uid:
        await bot.send(ev, f'[CQ:at,qq={uid}]{c.name}现在还不是你的女友哦。', at_sender=True)
        return
    else:
        if cid>10000:
            #检查金币是否足够分手
...
@sv.on_prefix(['YES','Yes','yes']) #需@
async def noblesend1(bot, ev: CQEvent):
    if ev.message[0].type == 'at':
        id2 = int(ev.message[0].data['qq'])
    else:
        await bot.finish(ev, '需要@指定接受对象')
    gid = ev.group_id
    uid = ev.user_id
    status = send_judger.get_accept_status(gid,id2,uid) #sender accepter交换顺序
    if status == None:
        msg = f'对方没有向你赠送女友'
        await bot.send(ev, msg ,at_sender=True)
        return
    else:
        send_judger.accept_on_off(gid,id2,uid,True) #sender accepter交换顺序
 
@sv.on_prefix(['NO','No','no']) #需@
async def noblesend2(bot, ev: CQEvent):
    if ev.message[0].type == 'at':
        id2 = int(ev.message[0].data['qq'])
    else:
        await bot.finish(ev, '需要@指定拒绝对象')
    gid = ev.group_id
    uid = ev.user_id
    status = send_judger.get_accept_status(gid,id2,uid) #sender accepter交换顺序
    if status == None:
        ...
        send_judger.accept_on_off(gid,id2,uid,False) #sender accepter交换顺序

@sv.on_prefix(r'支付')
async def pay(bot, ev: CQEvent):
    msg = str(ev.message)
    if not msg:
        await bot.send(ev, '格式错误。', at_sender=True)
        return
    match = re.match(r'(\d+)金币\[CQ:at,qq=(\d+)\]',msg)
    gid = ev.group_id
    uid = ev.user_id
    money = int(match.group(1))
    accepter = int(match.group(2))
    score_counter = ScoreCounter2()
    if score_counter._judge_score(gid, uid ,money) == 0:
        msg = f'[CQ:at,qq={uid}]您的金币不足'
    else:
        score_counter._reduce_score(gid,uid,money)
        score_counter._add_score(gid,accepter,money)
        msg = f'[CQ:at,qq={uid}][CQ:at,qq={accepter}]交易成功！'
    await bot.send(ev, msg)
